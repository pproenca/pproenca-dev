...
test.describe("Visual Regression", () => {
  ...
  test("post content light theme", async ({ postPage }) => {
    ...
    await expect(postPage.articleContent).toHaveScreenshot(
      "post-content-light.png",
    );
  });
  ...
  test("code block styling", async ({ postPage }) => {
    ...
    if (codeBlockCount > 0) {
      await expect(postPage.codeBlocks.first()).toHaveScreenshot(
        "code-block-light.png",
      );
    }
  });
